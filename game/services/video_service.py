...
class VideoService:
    ...
    def open_window(self):
        ...
        """
        pyray.init_window(constants.WIDTH, constants.HEIGHT, constants.CAPTION)
        pyray.set_target_fps(constants.FRAME_RATE)

    def _draw_grid(self):
        """Draws a grid on the screen."""
        for y in range(0, constants.HEIGHT, constants.CELL_SIZE):
            pyray.draw_line(0, y, constants.WIDTH, y, pyray.GRAY)
            
        for x in range(0, constants.WIDTH, constants.CELL_SIZE):
            pyray.draw_line(x, 0, x, constants.HEIGHT, pyray.GRAY)
